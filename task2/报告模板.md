...
![repository](C:\Users\12733\Desktop\学习\tasks\Geek_Appraisal\task2\image2.png)
...
1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？
   ...
   哈夫曼树每两个子节点对应一个父节点，共n-1次合并就会有2n-1个结点

2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？

   将0x41改为0x61；判断大小，大写在0x41到0x5A之间，小写在0x61到0x7A之间

3. 使用画图工具画出上述两个Huffman生成树的生成过程

   ![image3](C:\Users\12733\Desktop\学习\tasks\Geek_Appraisal\task2\image3.png)
